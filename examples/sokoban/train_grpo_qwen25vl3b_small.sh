...
EXPERIMENT_NAME="grpo_sokoban_qwen25vl3b_small"

BASEDIR=$(pwd)
SCRIPTDIR=$(dirname "$0")
EXPERIMENT_DIR=${BASEDIR}/exps/${PROJECT_NAME}/${EXPERIMENT_NAME}
SAVE_CHECKPOINT_DIR=${EXPERIMENT_DIR}/verl_checkpoints
DATASET_TRAIN=${SCRIPTDIR}/train_sokoban_vision_small.yaml
DATASET_VAL=${SCRIPTDIR}/val_sokoban_vision_small.yaml
agent_loop_config_path=${BASEDIR}/vagen/configs/agent.yaml
REF_MODEL_PATH=Qwen/Qwen2.5-VL-3B-Instruct
mkdir -p ${EXPERIMENT_DIR}


PYTHONUNBUFFERED=1 /venv/embench-vagen/bin/python -m vagen.main_ppo \
